...
import { Lock, Eye, Play, X } from "lucide-react";
import Reveal from "./Reveal";
import { MEDIA } from "../data";

function Tile({ item, revealed, onOpen, index }) {
    ...
    >
      {item.type === "image" ? (
        <img
          ...
          style={{ filter: revealed ? "none" : "blur(26px)", transform: revealed ? "none" : "scale(1.08)" }}
        />
      ) : (
        <video
          src={item.src}
      ...
      {item.type === "video" && revealed && (
        <span className="pointer-events-none absolute bottom-3 right-3 rounded-full bg-black/60 p-2 text-gold backdrop-blur">
          <Play size={14} />
        </span>
      )}
      {revealed && (
        <span className="pointer-events-none absolute inset-0 bg-gradient-to-t from-ink/50 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
      )}
    </button>
  );
}
          ...
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <div>
              ...
              <p className="mt-3 max-w-md font-sans text-sm font-light text-muted">
                Photos and clips are blurred for your discretion. Opt in to
                reveal.
              </p>
            </div>
            <button
        ...
        <div className="relative mt-12">
          <div className="grid auto-rows-[minmax(0,1fr)] grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
            {MEDIA.gallery.map((item, i) => (
              <Tile key={i} item={item} index={i} revealed={revealed} onOpen={setActive} />
            ))}
          </div>
